...
final class StoreController
{
    ...
    public function showForm(Request $request, Response $response): Response
    {
        ...
        if (isset($_SESSION['id'])) {
            //pasem informacio de la sesio a user
            $user = $this->userRepository->getUserByToken($_SESSION['id']);
            //posem la informacio a las credencials
            $credentials = array(
                'username' => $user->getUsername(),
                'picture' => $user->getPicture()
            );
        } else {
            //sino no esten logejats deixem les credencials a null
            $credentials = null;
        }
    ...
    public function buyGame(Request $request, Response $response): Response
    {
        ...
        //recorrem tots els jocs
        foreach ($games as &$game) {
            //mirem si el id del jocs correspon al id del joc que hem seleccionat
    ...
    public function myGames(Request $request, Response $response): Response
    {
        ...
        if (isset($_SESSION['id'])) {
            ...
            //recorre els jocs del usuari (guardats a la base de dades)
            foreach ($games as &$game) {
                //recorre els jocs de la api 
                foreach ($gamesAPI as &$gameAPI) {
                    //busquem el game id  de a api = al que volem
                    if ($gameAPI->gameID == $game['gameId']) {
                        //afegim jocs a una array buida per passar per pantalla
